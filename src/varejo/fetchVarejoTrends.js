...
async function fetchInterestOverTimeSerpApi(keywords, { dateFrom, dateTo }) {
  ...
  for (const kw of keywords) {
    try {
      const data = await serpApiRequest({ search_type: 'google_trends', q: kw, geo: 'BR', hl: 'pt-BR', date, data_type: 'TIMESERIES' });
...
async function fetchRelatedQueriesSerpApi(keyword, { dateFrom, dateTo }) {
  const date = `${dateFrom} ${dateTo}`;
  try {
    const data = await serpApiRequest({ search_type: 'google_trends', q: keyword, geo: 'BR', hl: 'pt-BR', date, data_type: 'RELATED_QUERIES' });
...
async function fetchRelatedTopicsSerpApi(keyword, { dateFrom, dateTo }) {
  const date = `${dateFrom} ${dateTo}`;
  try {
    const data = await serpApiRequest({ search_type: 'google_trends', q: keyword, geo: 'BR', hl: 'pt-BR', date, data_type: 'RELATED_TOPICS' });
